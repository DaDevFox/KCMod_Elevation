...
namespace Elevation
{
    ...
    {
        public static List<string> scaffoldedCells = new List<string>();
        ...
        private void RaiseTerrain()
        {
            ...
            if (ElevationManager.TryProcessElevationChange(cell, 1))
            {
                ...
                KingdomLog.TryLog("elevationraise", "With momentous effort, a crew has managed to artificially raise terrain, highness.", KingdomLog.LogStatus.Neutral, 1f, cell.Center, false, this.b.LandMass());

                string key = $"{cell.x}_{cell.z}";

                if(Dugout.dugoutCells.Contains(key))
                    Dugout.dugoutCells.Remove(key);
                else
                    scaffoldedCells.Add(key);
            }
